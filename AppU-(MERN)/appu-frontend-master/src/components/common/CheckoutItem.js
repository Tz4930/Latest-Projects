  ...
  IncrementItem = () => {
    if(this.state.quantity >= this.state.max) {
        ...
        if(this.props.variantId) {
          this.props.getValue({id:this.props.id, variantId: this.props.variantId, quantity: (this.state.quantity + 1 ), price: this.props.price});
        } else {
    	...
    	<div className="gold-members p-2 border-bottom">
           ...
           <div className="media">
              ...
              <div className="media-body">
                 <p className="mt-1 mb-0 text-black">{this.props.itemName}
                 {this.props.variantId && <span> - {this.props.variantId}</span>}
                 </p>
              </div>
           </div>
        </div>
